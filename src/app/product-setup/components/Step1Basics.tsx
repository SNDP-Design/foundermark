...
export default function Step1Basics({ form }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-[17px] font-bold mb-[4px]" style={{ color: '#111111' }}>Tell us about your product</h2>
        <p className="text-[13px]" style={{ color: '#6b7280' }}>This information shapes every piece of content the AI generates for you.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label className="block text-[12px] font-semibold uppercase tracking-[0.4px] mb-[6px]" style={{ color: '#6b7280' }} htmlFor="productName">
            Product name
          </label>
          ...
          {errors.productName && <p className="text-[11px] mt-1" style={{ color: '#dc2626' }}>{errors.productName.message}</p>}
        </div>
        <div>
          <label className="block text-[12px] font-semibold uppercase tracking-[0.4px] mb-[6px]" style={{ color: '#6b7280' }} htmlFor="websiteUrl">Website URL</label>
          <input id="websiteUrl" type="url" className="input-base" placeholder="https://yourproduct.com" {...register('websiteUrl')} />
        </div>
      </div>

      <div>
        <label className="block text-[12px] font-semibold uppercase tracking-[0.4px] mb-[6px]" style={{ color: '#6b7280' }} htmlFor="description">
          Product description
        </label>
        <p className="text-[11px] mb-[6px]" style={{ color: '#9ca3af' }}>2–4 sentences about what your product does and who it's for</p>
        <textarea
        ...
        />
        {errors.description && <p className="text-[11px] mt-1" style={{ color: '#dc2626' }}>{errors.description.message}</p>}
      </div>
      ...
      <div>
        ...
        <p className="text-[11px] mb-2" style={{ color: '#9ca3af' }}>Helps the AI use the right vocabulary and benchmarks</p>
          ...
          {industries.map((ind) => (
            <label key={ind.key} className="cursor-pointer">
              ...
              <span
                ...
                style={{
                  border: `1px solid ${form.watch('industry') === ind.value ? '#9ca3af' : '#e5e7eb'}`,
                  background: form.watch('industry') === ind.value ? '#f3f4f6' : '#fafafa',
                  color: form.watch('industry') === ind.value ? '#111111' : '#6b7280',
                }}
              >
                {ind.label}
              </span>
        ...
        {errors.industry && <p className="text-[11px] mt-1" style={{ color: '#dc2626' }}>{errors.industry.message}</p>}
      </div>
    </div>
  );
}